...
od_fsemana <- od_bike_group %>% filter(day_type == "final_de_semana") %>%
  ...
  arrange(desc(trips_n)) %>%
  ungroup() %>%
  left_join(unique(od_bike_group_dt, by = c("initial_station_name", "final_station_name")) %>% 
              select(initial_station_name, final_station_name, geometry)) %>%
  st_sf()
...
st_write(od_fsemana, "../../data/smtr_malha_cicloviaria/explore_od_tembici/viagens_por_od/viagens_od_fsemana.gpkg", append = FALSE)
